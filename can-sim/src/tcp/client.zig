...
fn handler(client: net.Stream, doRead: bool) void {
    if (doRead) {
        ...
        writeBytes(client, "TestDataFrame ", "ecu");
    }
}

fn connect(clName: []const u8, doRead: bool) !void {
    ...
    while (true) {
        handler(client, doRead);
    }

    defer client.close();
}

fn read(stream: net.Stream, clName: []const u8) !usize {
